...
function switchView(targetView) {
  const views = [viewLanding, viewIntro, viewAuth, viewMain];
  views.forEach(view => {
    if (!view) return;
    if (view === targetView) {
      view.classList.remove('hidden', 'opacity-0', 'pointer-events-none');
  ...
  if (targetView === viewIntro) {
    setTimeout(() => {
      ...
      if (introContent) introContent.classList.replace('translate-y-10', 'translate-y-0');
    }, 50);
  }
  if (targetView === viewAuth) {
    setTimeout(() => {
      const authContent = document.getElementById('auth-modal-content');
      if (authContent) authContent.classList.replace('translate-y-10', 'translate-y-0');
    }, 50);
  }
  if (targetView === viewMain) {
    if (isFirstMainView) {
      ...
      if (!hasFolders) {
        setTimeout(() => {
          if (tutorialOverlay) {
            ...
            tutorialOverlay.classList.remove('opacity-0', 'pointer-events-none');
            tutorialOverlay.classList.add('opacity-100', 'pointer-events-auto');
          ...
          if (tutorialTooltip) {
            tutorialTooltip.classList.remove('hidden', 'opacity-0', 'pointer-events-none');
...
async function ensureDefaultWorkspace() {
  if (Object.keys(folders).length === 0) {
    const id = currentUser ? generateUUID() : 'folder_1';
    const newFolder = {
      name: '자유 탐구',
      emoji: '💡',
      level: 3,
    ...
    if (currentUser && window.supabaseAPI) {
      try {
        ...
        console.error("Failed to insert default workspace into DB:", e);
      }
    } else if (isGuest) {
      persistGuestData();
    }
...
btnAuthGuest.addEventListener('click', async () => {
  isGuest = true;
  currentUser = null;
  resetGuestData();
...
// ================================================
// THEME TOGGLE (Legacy - uses settings modal buttons now)
// ================================================
if (btnThemeToggle) {
...
function closeTutorial() {
  const tutorialBackdrop = document.getElementById('tutorial-backdrop');
  if (tutorialBackdrop) {
    tutorialBackdrop.classList.remove('pointer-events-auto');
    tutorialBackdrop.classList.add('pointer-events-none');
  }

  if (tutorialOverlay) {
    tutorialOverlay.classList.remove('opacity-100', 'pointer-events-auto');
  ...
  if (tutorialTooltip) {
    tutorialTooltip.classList.remove('opacity-100', 'pointer-events-auto');
    tutorialTooltip.classList.add('opacity-0', 'pointer-events-none');
    setTimeout(() => {
      ...
      if (typeof window.supabaseAPI.onAuthStateChange === 'function') {
        window.supabaseAPI.onAuthStateChange(async (event, session) => {
          if (session && session.user) {
            ...
            // Clean hash or search parameters from URL after successful OAuth exchange
